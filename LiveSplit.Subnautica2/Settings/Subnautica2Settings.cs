        ...
        private static XmlElement CreateSplitElement(XmlDocument document, Subnautica2Split split)
        {
            ...
            switch (split)
            {
                ...
                case CraftSplit craftSplit:
                    ...
                    break;
                case BuildSplit buildSplit:
                    xmlValue.InnerText = buildSplit.Buildable.ToString();
                    break;
                default:
        ...
        private Subnautica2Split ReadSplitNode(XmlNode splitNode)
        {
            ...
            switch (splitName)
            {
                case SplitName.Inventory:
                    var item = Subnautica2SplitSetting.GetInventoryItem(values[0]);
                    split = new ItemSplit(item, onlySplitOnce, isSubCondition);
                    if (values.Length >= 4)
                    {
                ...
                case SplitName.Blueprint:
                    var blueprint = Subnautica2SplitSetting.GetUnlockable(value);
                    split = new BlueprintSplit(blueprint, onlySplitOnce, isSubCondition);                    
                    break;
                ...
                case SplitName.Craft:
                    var craftable = Subnautica2SplitSetting.GetCraftable(value);
                    split = new CraftSplit(craftable, onlySplitOnce, isSubCondition);
                    break;

                case SplitName.Build:
                    var buildable = Subnautica2SplitSetting.GetBuildable(value);
                    split = new BuildSplit(buildable, onlySplitOnce, isSubCondition);
                    break;
